...
A grounded reference on what xete is, how it's built, and the principles behind it. Canonical source of truth for program identity and infra is the maintainer's private project-state record, which is not part of this repository and is not published. Never infer program identity from app-code constants — verify against that record and on-chain. This doc is orientation, not a substitute for it. **No secrets/keys belong here — this folder is indexed.**
