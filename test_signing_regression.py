...
def test_a_name_the_registry_cannot_hold_is_refused_unsigned(alias_server, monkeypatch):
    ...
    claim of such a name can only burn a fee. txguard refuses it before any signature
    over the transaction exists — which is what closes the fee burn that
    ...
    try:
        txguard_mod.inspect_alias_claim(
            tx_b64,
            expect_fee_payer=Pubkey.from_string(pubkey),
            expect_name="not a name!",
            quoted_lamports=0,
            treasury=Pubkey.from_string(XETE_TREASURY),
        )
    except txguard_mod.TransactionRejected as e:
